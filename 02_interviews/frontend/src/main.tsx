...
const Shell=({children}:{children:React.ReactNode})=><div className="shell"><header><Link className="button secondary brand-link" to="/admin/sessions">Interview Canvas</Link><nav><Link className="button secondary" to="/admin/sessions">Sessions</Link><button className="secondary" onClick={async()=>{await service.logout().catch(()=>undefined);localStorage.clear();location.href="/login"}}>Log out</button></nav></header>{children}</div>;
...
function Login(){const nav=useNavigate();const [email,setEmail]=useState("");const [password,setPassword]=useState("");const [error,setError]=useState("");async function submit(e:React.FormEvent){e.preventDefault();try{const result=await service.login(email,password);localStorage.setItem("admin_token",result.access_token);nav("/admin/sessions")}catch(e){setError(String(e))}}const googleSuccess=(user:{id:string;email:string;role:string})=>{localStorage.setItem("admin_token","session-cookie");nav("/admin/sessions")};return <main className="card narrow"><h1>Administrator login</h1><form onSubmit={submit}><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required/></label>{error&&<p className="error">{error}</p>}<button>Sign in</button></form><hr/><GoogleSignIn onSuccess={googleSuccess}/></main>}
...
export function App(){return <Routes><Route path="/login" element={<Login/>}/><Route path="/admin" element={<Navigate to="/admin/sessions" replace/>}/><Route path="/admin/sessions" element={<Protected><SessionDashboard/></Protected>}/><Route path="/admin/sessions/:sessionId" element={<Protected><SessionDetail/></Protected>}/><Route path="/admin/sessions/:sessionId/evaluation" element={<Protected><SessionEvaluation/></Protected>}/><Route path="/invite/:token" element={<Invite/>}/><Route path="/session/:sessionId" element={<LiveWorkspace/>}/><Route path="*" element={<Navigate to="/admin/sessions" replace/>}/></Routes>}
